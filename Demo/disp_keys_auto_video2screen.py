...
def load_and_scale_image(path, size):
    try:
        ...
        print(f"画像の読み込みに失敗しました: {path} ({exc})")
        return None
...
auto_files = get_auto_files("./auto")
...
def start_video(video_path):
    ...
    try:
        ...
        if not args.test:
            vlc_args.append("--qt-fullscreen-screennumber=1")
        ...
        print(f"動画ファイル {video_path} をVLCで再生開始しました。")
        return True
    except FileNotFoundError:
        print("エラー: VLC media playerが見つかりません。パスを確認してください。")
    except Exception as exc:
        print(f"動画再生中にエラーが発生しました: {exc}")
    return False
...
running = True
while running:
    if is_video_playing and video_process and video_process.poll() is not None:
        is_video_playing = False
        video_process = None
        print("動画再生が終了しました。")
        advance_auto_after_video()
